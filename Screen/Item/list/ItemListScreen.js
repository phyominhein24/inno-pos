...
const ItemListScreen = ({ navigation, route }) => {
  ...
  const deleteItem = async () => {
    setIsLoading(true);
    const result = await itemService.destory(dispatch, selectedItem?.id);
  ...
  const handleEndReached = async () => {
    ...
    const result = await itemService.index(dispatch, { ...paginateParams, page: nextPage });
    if (result.status === 200 && result.data.items.length < paginateParams.per_page) {
      setHasMore(false);
    }
  };
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.item}>
              ...
              <Image
                source={item?.photo ? { uri: item.photo } : require('../../../assets/default_profile_fix.png')}
            ...
            <View style={styles.iconContainer}>
              <TouchableOpacity onPress={() => navigation.navigate('ItemUpdate', { data: item })}>
        ...
        // onEndReached={handleEndReached}  // Triggered when item reaches the bottom
        // onEndReachedThreshold={0.5}  // When item is 50% from the bottom, trigger fetch
        // ListFooterComponent={isLoading ? <ActivityIndicator size="large" /> : null}  // Loading indicator at bottom
      />

      <TouchableOpacity style={styles.addButton} onPress={() => navigation.navigate('ItemCreate')}>
