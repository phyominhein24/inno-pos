...
const InvoiceCreateScreen = ({ navigation, route }) => {
  ...
  const [payload, setPayload] = useState(invoicePayload.store);
  ...
  const submitInvoice = async () => {
    try {
      setLoading(true);
      const formData = formBuilder(payload, invoicePayload.store);
      const response = await invoiceService.store( formData, dispatch);
