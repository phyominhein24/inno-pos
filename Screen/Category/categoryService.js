...
import { index, update } from "./categorySlice";
...
export const categoryService = {
    ...
    destory: async (dispatch, id) => {
        const response = await delRequest(`${endpoints.category}/${id}`);
        await httpServiceHandler(dispatch, response);
    ...
    exportexcel: async (dispatch, params) => {
        ...
        const response = await featchGetRequest(`${baseURL}/${endpoints.category}/exportexcel`, params)        
        if (response.status === 200) {            
            dispatch(updateNotification({
                variant : 'success',
    ...
    exportexcelparams: async (dispatch, params) => {
        dispatch(updateNotification2({variant: "success"}))
        const response = await featchGetRequest(`${baseURL}/${endpoints.category}/exportexcelparams`, params)
    ...
    exportpdf: async (dispatch, params) => {
        dispatch(updateNotification2({variant: "success"}))
        const response = await featchGetRequest(`${baseURL}/${endpoints.category}/exportpdf`, params, "pdf")
    ...
    exportpdfparams: async (dispatch, params) => {
        dispatch(updateNotification2({variant: "success"}))
        const response = await featchGetRequest(`${baseURL}/${endpoints.category}/exportpdfparams`, params, "pdf")
    ...
    import: async (payload, dispatch) => {
        dispatch(updateNotification2({variant: "success"}))
        const response = await postRequest(`${endpoints.category}/import`, payload);
        await httpServiceHandler(dispatch, response);
        dispatch(removeNotification2());
        return response
    },
